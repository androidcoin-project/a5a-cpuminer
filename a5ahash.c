...
int scanhash_a5a_hash(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
    uint64_t max_nonce, unsigned long *hashes_done)
{
    ...
    uint32_t hash[6] __attribute__((aligned(32)));
    uint8_t bhash[5][64] __attribute__((aligned(32)));
    ...
    mpz_t a5api, a5asw, product, bns0, bns1;
    mpf_t a5afpi, a5afpi0, mpt1, mpt2, mptmp, mpten;
    
    mpz_inits(a5api, a5asw, bns0, bns1, NULL);
    ...
    mpf_set_str(a5afpi0, "0.b7bfc6837e20bdb22653f1fc419f6bc33ca80eb65b7b0246f7f3b65689560aea1a2f2fd95f254d68c", 16);
    ...
    do {
        ...
        sph_ripemd160_close(&ctx2_ripemd, (void*)(bhash[4]));
	...
	mpzscale = 1;
	    ...
	    mpf_sub(mpt1, mpt1, mpt2);
            ...
            mpf_mul(a5afpi, a5afpi, mptmp);
	    mpz_set_f(a5api, a5afpi);
            mpz_add(a5api,a5api,a5asw);
	...
	for (i = 31; i >= 0; i--) {
     ...
     pdata[19] = n;
out:
	mpf_set_prec_raw(a5afpi, prec0);
	mpf_set_prec_raw(a5afpi0, prec0);
	...
	mpf_clear(mpt1);
	mpf_clear(mpt2);
	mpz_clears(a5api, a5asw, product, bns0, bns1, NULL);

    *hashes_done = n - first_nonce + 1;
    return rc;
}

int scanhash_a5a_hash_t(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
