...
static enum algos opt_algo = ALGO_A5A;
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	if (opt_algo == ALGO_SCRYPT || opt_algo == ALGO_A5A)
	...
	while (1) {
		...
		if (fhash_test)
		{
			...
			if(opt_cpu_eff == 100) { fcpu_dec = false; }
				...
				scanhash_a5a_hash(thr_id, work.data, work.target, work.data[19] + 1000, &hashes_done);
				...
				for (i_=1; i_<=100; i_++)
				{
					...
					hashes_done = 0;
					gettimeofday(&ttest_start, NULL);
					scanhash_a5a_hash_t(thr_id, work.data, work.target, work.data[19] + 100, &hashes_done, cpu_dec_time);
					gettimeofday(&ttest_end, NULL);
					timeval_subtract(&tdiff, &ttest_end, &ttest_start);
		...
		case ALGO_A5A:
			rc = ( fcpu_dec ? scanhash_a5a_hash_t(thr_id, work.data, work.target,
			                      max_nonce, &hashes_done, cpu_dec_time) : 
			                  scanhash_a5a_hash(thr_id, work.data, work.target,
			                      max_nonce, &hashes_done) );
			break;
